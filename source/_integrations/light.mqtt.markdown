...
This schema supports on/off, brightness, RGB colors, XY colors, color temperature, transitions, short/long flashing and effects. Following units and value ranges are passed to/obtained from the system:
* RGB colors, value from range: 0 - 255
* Brightness, value from range: 0 - 255
* color temperature: Mireds. To see how to convert Mireds to/from Kelvin, see example below
...
brightness_template:
  ...
  type: string
color_temp_template:
  description: "[Template](/docs/configuration/templating/#processing-incoming-data) to extract color temperature from the state payload value. The value is expected in Mireds unit."
  required: false
  type: string
command_off_template:
  ...
  type: string
command_on_template:
  description: "The [template](/docs/configuration/templating/#processing-incoming-data) for *on* state changes. Available variables: `state`, `brightness`, `temp` (in Mireds), `red`, `green`, `blue`, `flash`, `transition` and `effect`. The `temp` value is in Mireds."
  required: true
  type: string
command_topic:
  description: The MQTT topic to publish commands to change the light’s state.
...
Max_mireds and min_mireds set color temperature boundaries and sets them to 3000K - 6500K. Notice the same limits are applied in `command_on_template`, but in Kelvin units this time. It's due to conversion from Mireds to Kelving which causes exceeding boundary values accepted by the device.
